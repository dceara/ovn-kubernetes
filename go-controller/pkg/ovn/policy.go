...
func (oc *Controller) createMulticastAllowPolicy(ns string, nsInfo *namespaceInfo) error {
	...
	match = getACLMatch(portGroupName,
...
func (oc *Controller) createDefaultDenyMulticastPolicy() error {
	...
	match := getACLMatch(clusterPortGroupMcastDenyName, mcastMatch, knet.PolicyTypeEgress)
	...
	match = getACLMatch(clusterPortGroupMcastDenyName, mcastMatch, knet.PolicyTypeIngress)
