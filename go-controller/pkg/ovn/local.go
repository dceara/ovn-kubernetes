...
func (lc *LocalController) Run(wg *sync.WaitGroup) error {
	...
	if config.OVNKubernetesFeature.EnableEgressIP {
		...
		// sync.
		if err := lc.oc.WatchEgressIPNamespaces(); err != nil {
			return err
		}
		if err := lc.oc.WatchEgressIPPods(); err != nil {
			return err
		}
		if err := lc.oc.WatchEgressNodes(); err != nil {
			return err
		}
		if err := lc.oc.WatchEgressIP(); err != nil {
			return err
		}
		if util.PlatformTypeIsEgressIPCloudProvider() {
			if err := lc.oc.WatchCloudPrivateIPConfig(); err != nil {
				return err
			}
		}
	}
	...
	if config.OVNKubernetesFeature.EnableEgressFirewall {
		...
		err = lc.oc.WatchEgressFirewall()
		if err != nil {
			panic(fmt.Sprintf("Error watching egress firewall for node %s: %v", node.Name, err))
		}
	}
...
func (lc *LocalController) WatchNodes() {
	if _, err := lc.oc.watchFactory.AddNodeHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			err = lc.oc.SetupMaster(make([]string, 0), util.GetNodeId(node))
			if err != nil {
			...
			if err := lc.oc.WatchNamespaces(); err != nil {
				panic(fmt.Sprintf("Failed to watch namespaces, error: %v", err))
			}

			if err := lc.oc.WatchPods(); err != nil {
				panic(fmt.Sprintf("Failed to watch namespaces, error: %v", err))
			}

			// WatchNetworkPolicy depends on WatchPods and WatchNamespaces
			if err := lc.oc.WatchNetworkPolicy(); err != nil {
				panic(fmt.Sprintf("Failed to watch namespaces, error: %v", err))
			}
			...
			if err != nil {
				...
				for _, pod := range pods.Items {
					pod := pod
					if lc.oc.isPodRelevant(&pod) {
						klog.V(5).Infof("Adding pod %s/%s to retryPods", pod.Namespace, pod.Name)
						lc.oc.retryPods.addRetryObjWithAdd(&pod)
					}
				}
				lc.oc.retryPods.requestRetryObjs()
			}
		...
		panic(fmt.Sprintf("Error watching nodes: %v", err))
	}
}
