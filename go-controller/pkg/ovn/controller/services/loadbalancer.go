...
func getTemplatesFromRules(rules []LBRule) TemplateMap {
...
func EnsureLBs(nbClient libovsdbclient.Client, service *corev1.Service, existingCacheLBs []LB, LBs []LB) error {
	...
	ops, err = svcCreateTemplateVarOps(nbClient, ops, toNBTemplateList(newlbs))
	if err != nil {
		return err
	}

	ops, err = svcCreateOrUpdateTemplateVarOps(nbClient, ops, toNBTemplateList(existinglbs))
	if err != nil {
