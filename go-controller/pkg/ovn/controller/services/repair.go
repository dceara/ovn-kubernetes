...
func (r *repair) runBeforeSync() {
	...
	staleTemplates := staleTemplateNames.UnsortedList()
	deleteTemplateVar := &nbdb.ChassisTemplateVar{
		Variables: make(map[string]string, len(staleTemplates)),
	}
	for _, templateName := range staleTemplates {
		deleteTemplateVar.Variables[templateName] = ""
	}
	if err := libovsdbops.DeleteChassisTemplateVarValues(r.nbClient, deleteTemplateVar, func(item *nbdb.ChassisTemplateVar) bool {
		for _, templateName := range staleTemplates {
