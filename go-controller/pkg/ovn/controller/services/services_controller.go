...
func (c *Controller) initTopLevelCache() error {
	...
	allTemplates, err := libovsdbops.ListTemplates(c.nbClient)
	if err != nil {
...
func (c *Controller) RequestFullSync() {
	...
	if c.useTemplates {
		nodeIPTemplateMap := libovsdbops.TemplateMap{}
		if c.nodeTracker.nodeIPV4Template.Len() > 0 {
			nodeIPTemplateMap[c.nodeTracker.nodeIPV4Template.Name] = c.nodeTracker.nodeIPV4Template
		}
		if c.nodeTracker.nodeIPV6Template.Len() > 0 {
			nodeIPTemplateMap[c.nodeTracker.nodeIPV6Template.Name] = c.nodeTracker.nodeIPV6Template
		}
